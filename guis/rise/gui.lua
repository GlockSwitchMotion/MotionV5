...
local getcustomassets = {
	['motionvape/assets/rise/slice.png'] = 'rbxasset://risesix/slice.png',
	['motionvape/assets/rise/blur.png'] = 'rbxasset://risesix/blur.png',
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('motionvape/assets/new/blur.png')
...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/YourGitHub/MotionV4/'..readfile('motionvape/profiles/commit.txt')..'/'..select(1, path:gsub('motionvape/', '')), true)
		end)
...
local function writeFont()
	...
	writefile('motionvape/assets/rise/risefont.json', httpService:JSONEncode({
		name = 'ProductSans',
		faces = {
			{style = 'normal', assetId = getcustomasset('motionvape/assets/rise/SF-Pro-Rounded-Light.otf'), name = 'Light', weight = 300},
			{style = 'normal', assetId = getcustomasset('motionvape/assets/rise/SF-Pro-Rounded-Regular.otf'), name = 'Regular', weight = 400},
			{style = 'normal', assetId = getcustomasset('motionvape/assets/rise/SF-Pro-Rounded-Medium.otf'), name = 'Medium', weight = 500},
			{style = 'normal', assetId = getcustomasset('motionvape/assets/rise/Icon-1.ttf'), name = 'Icon1', weight = 600},
			{style = 'normal', assetId = getcustomasset('motionvape/assets/rise/Icon-3.ttf'), name = 'Icon3', weight = 800}
		}
	}))
	return getcustomasset('motionvape/assets/rise/risefont.json')
end

if inputService.TouchEnabled then
	writefile('motionvape/profiles/gui.txt', 'new')
	return
end
	...
	Function = function(val, mouse)
		if mouse then
			...
			if shared.VapeDeveloper then
				...
				loadstring(game:HttpGet('https://raw.githubusercontent.com/YourGitHub/MotionV4/'..readfile('motionvape/profiles/commit.txt')..'/loader.lua', true))()
			end
		end
	end
})
		...
		for i, v in mainapi.Modules do
			if v.Enabled or table.find(found, i) then
				...
				if interfacebar.Enabled then
					...
					holderline.Image = getcustomasset('motionvape/assets/rise/slice.png')
